...
;its blocking/deblocking algorithm, or verified writes). To
...
;
;Xmodem  versions prior to 2.10 would fail in some cases during
;reception with checksums, with a "Lost Blocks" error,if
;XMODEM was started before the sender (e.g. TeraTerm) was
;started. Xmodem will send a NAK every few seconds to tell the
;sender to start sending, and to use checksums. Xmodem will do
;this until it receives a byte from the sender. Before
;sending the file, TeraTerm attempts to flush its serial port
;to get rid of any stray NAKs by repeatedly reading from its
;serial port until the serial port indicates that it has no
;data available. This will normally flush out an accumulated
;NAKs that Xmodem sent while trying to establish connection for
;file reception using checksums. However, this fails if the PC
;hardware or OS does not immediately report that a new
;character is waiting after TeraTerm's flush loop reads a NAK.
;The result will be a extra NAK(s) in its serial port buffer,
;causing TeraTerm to  resend the first block. This will cause
;an Xmodem synchronization error later on, the first time that
;Xmodem holds off ACKing a block (e.g. to write to disk)
;because there will be an extra ACK in the queue. The solution
;(in Xmodem version 2.10) is to wait to see if the serial port
;is idle for a half second after the very first block. If not,
;then Xmodem will receive the resent block 1 without having
;sent an ACK for the first one. From there on, synchronization
;is correct.
;
...
; Thanks to Bill Gates for his slick recursive decimal-print
;   subroutine.
;==============================================================
;Revision History:
; 2.10 30 May 2026  M. Eberhard
;  Check for and correct NAK sync error on the first block
;  received, if using checksums. Fix typo in "can't sync" error
;  message. Use Bill Gates's decimal-print subroutine
;  (called PDEC16 here).
;
...
;---------------------------
VERSION	equ	0210h	;high byte = major revision number
...
;Timeout values in half-seconds. Values in parenthesis are
;Xmodem standard values.

SOHTO	equ	10*2	;(10 secs)sender to send SOH 
NAKTO	equ	90*2	;(90 secs)receiver to send init NAK
ACKTO	equ	60*2	;(60 secs)receiver to ACK (or NAK)
			;(time to write to disk)
IDLTO	equ	1	;initia block idle timeeout
...
;-----------------------------------------
GETACK:	mvi	a,ACKTO		;ACK-wait timeout value
...
;  BUFMAX = Max address in BUFFER/256
;  XBLOCK = previous 16-bit block number
;On Exit (valid block):
...
RXRTRY:	mvi	a,SOHTO		;Timeout for SOH
	call	RXBYTE

GOTCHA:	cpi	SOH		;Did we get an SOH?
...
;---------------------------------------------------------
;Special case for the first block received. Wait here
;to see that the serial port is idle. If not, receive
;another block without having sent an ACK first. If this
;is block 1 again, then the sender is out of sync due
;to the initial NAKs. If it's anything else, its a failure
;  hl=next buffer address
;---------------------------------------------------------
	lda	CRCFLG		;CRC mode?
	ora	a
	jnz	NOTCSM		;no problem then

	xchg			;save buffer address in de
	lhld	XBLOCK		;16-bit Xmodem block #
	mov	a,h		;first block?
	ora	l
	jnz	NOT1ST

;first block (or the 65536th block...) See if the line is idle

	lxi	h,ISIDLE
	shld	RTORET		;return there if idle

	mvi	a,IDLTO+1	;timer high byte
	call	RXBYTE		;see if anything's waiting

;Returns here only if a byte was received

	lxi	h,RXSERR	;Timeout return address
	shld	RTORET		;..for RXBYT1 and RXBYTE
	xchg			;restore hl
	jmp	GOTCHA		;go see if it's an SOH

;restore return address for transfer

ISIDLE:	lxi	h,RXSERR	;Timeout return address
	shld	RTORET		;..for RXBYT1 and RXBYTE

NOT1ST:	xchg			;restore hl
NOTCSM:
...
;***Subroutine***********************************************
;Print hl in decimal on the console, leading zeros suppressed
;(This tight little subroutine was written by Bill Gates.)
;Trashes all registers
;************************************************************
PDEC16:	lxi	b,-10		;dividing by 10

GETDIG:	mov	d,b		;start DE (count of 10s) at -1
	mov	e,b

DIVLUP:	dad	b		;subtract 10 from H
	inx	d		;count 10's
	jc	DIVLUP		;carry set until < 0

	push	h		;save L = remainder-10
	xchg			;HL=quotient
	mov	a,h		;quotient reach zero?
	ora	l
	cnz	GETDIG		;no, recurse to get another digit

	mvi	a,'0'+10	;ASCII offset, extra 10 taken out
	pop	b		;C=binary digit to display-10
	add	c		;A=ASCII digit to display
...
;Trashes psw,bc
;**********************************************************
GTMODE:	mvi	b,NAKTO/2	;Long timeout
...
;    123456789012345678901234567890123456789012345678901234567890123
 ...
 db ((VERSION AND 00F0h)/16)+'0',(VERSION AND 0Fh)+'0'
	db	' By M.Eberhard',CR
 db '=========================',CR
